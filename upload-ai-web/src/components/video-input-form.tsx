...
export function VideoInputForm({ onVideoUploaded }: VideoInputFormProps) {
  ...
  async function handleUploadVideo(event: FormEvent<HTMLFormElement>) {
    ...
    const reponse = await api.post('/videos', data);

    const videoId = reponse.data.video.id;
  ...
  return (
    <form onSubmit={handleUploadVideo} className="space-y-6">
      <label
       ...
       data-[previewURL=true]:border-2
       data-[previewURL=true]:border-solid'
      >
        {previewURL ? (
          <video src={previewURL} controls={false} className="pointer-events-none rounded-lg" />
        ) : (
          <>
